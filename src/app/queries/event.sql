SELECT
  ...
  CONCAT('https://my.pureheart.org/ministryplatformapi/files/', COALESCE(F.Unique_Name, '')) AS Event_Image_URL,
  CONCAT(
    (SELECT COALESCE(Base_URI, '') FROM dp_Domains WHERE Domain_ID = 1),
    (SELECT COALESCE(CAST(Page_ID AS NVARCHAR(255)), '') FROM (SELECT TOP 1 Page_ID FROM dp_Pages PG WHERE PG.Display_Name = 'Events') AS PageSubQuery),
    '/', E.Event_ID
  ) AS Event_Path,
  COALESCE((
    SELECT DISTINCT
...
LEFT JOIN dp_Files F ON F.Record_ID = E.Event_ID AND F.Page_ID = 308
